...
function printInvoice(invoice: InvoiceDetail) {
  ...
  if (!popup) {
    window.alert('Please allow pop-ups to print or save this invoice as a PDF.');
    return;
  }
  const rows = invoice.items.map((item) => `<tr><td>${escapeHtml(item.productName)}<small>${escapeHtml(item.productSku)}</small></td><td>${item.quantity}</td><td>${amount(item.unitPrice, currency)}</td><td>${amount(item.lineTotal, currency)}</td></tr>`).join('');
  popup.document.write(`<!doctype html><html><head><title>${escapeHtml(invoice.invoiceNumber)} · AutoBiz</title><style>
    @page{size:A4;margin:16mm}*{box-sizing:border-box}body{font-family:Arial,sans-serif;color:#18353b;margin:0;font-size:13px}header{display:flex;justify-content:space-between;border-bottom:3px solid #1e7a68;padding-bottom:22px;margin-bottom:28px}h1{font-size:30px;margin:0 0 8px;letter-spacing:-1px}h2{font-size:15px;margin:0 0 7px}p{margin:4px 0;color:#64777b}small{display:block;color:#829094;font-size:10px;margin-top:4px}table{border-collapse:collapse;width:100%;margin-top:24px}th{text-align:left;color:#64777b;font-size:10px;text-transform:uppercase;letter-spacing:1px;padding:10px 8px;border-bottom:1px solid #d7e1de}td{padding:14px 8px;border-bottom:1px solid #e8efed}th:not(:first-child),td:not(:first-child){text-align:right}.summary{margin:28px 0 0 auto;width:290px}.line{display:flex;justify-content:space-between;padding:7px 0;color:#64777b}.total{border-top:2px solid #18353b;margin-top:7px;padding-top:12px;font-size:18px;font-weight:bold;color:#18353b}.meta{display:flex;justify-content:space-between;gap:24px;margin-bottom:22px}.meta>div{flex:1}.status{display:inline-block;border-radius:999px;background:#e6f1ed;color:#1e7a68;padding:5px 10px;font-size:11px;font-weight:bold;text-transform:capitalize}.footer{margin-top:52px;padding-top:16px;border-top:1px solid #d7e1de;color:#829094;font-size:11px}
  </style></head><body><header><div><h1>${escapeHtml(invoice.business.name || 'AutoBiz business')}</h1><p>${escapeHtml(invoice.business.type || '')}${invoice.business.city ? ` · ${escapeHtml(invoice.business.city)}` : ''}</p></div><div style="text-align:right"><div style="font-size:22px;font-weight:bold">INVOICE</div><p>${escapeHtml(invoice.invoiceNumber)}</p><p>${dateLabel(invoice.invoiceDate)}</p></div></header><div class="meta"><div><h2>Bill to</h2><p><strong>${escapeHtml(invoice.customerName)}</strong></p><p>${escapeHtml(invoice.customerPhone || 'No phone provided')}</p><p>${escapeHtml(invoice.customerAddress || 'No address provided')}</p></div><div style="text-align:right"><h2>Payment</h2><p><span class="status">${escapeHtml(invoice.paymentStatus)}</span></p><p>${escapeHtml(invoice.paymentMethod.toUpperCase())}</p><p>${amount(invoice.amountPaid, currency)} paid · ${amount(invoice.balanceDue, currency)} due</p></div></div><table><thead><tr><th>Item</th><th>Qty</th><th>Unit price</th><th>Line total</th></tr></thead><tbody>${rows}</tbody></table><div class="summary"><div class="line"><span>Subtotal</span><strong>${amount(invoice.subtotal, currency)}</strong></div><div class="line"><span>Discount ${invoice.discountType === 'percentage' ? `(${invoice.discountValue}%)` : ''}</span><strong>−${amount(invoice.discountAmount, currency)}</strong></div><div class="line total"><span>Total</span><strong>${amount(invoice.grandTotal, currency)}</strong></div></div><div class="footer">Generated by AutoBiz · This invoice reflects the saved transaction record.</div></body></html>`);
