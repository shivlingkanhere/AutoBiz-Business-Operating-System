...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <div className="noise min-h-[100dvh] bg-background"><div className={`hidden md:block ${collapsed ? 'w-[82px]' : 'w-[258px]'} shrink-0`} /><div className="min-h-[100dvh] flex-1"><div className="sticky top-0 z-30 flex h-[72px] items-center justify-between border-b border-border/80 bg-background/90 px-5 backdrop-blur-md md:px-8"><div className="flex items-center gap-3"><button onClick={() => setMobileOpen(true)} className="focus-ring rounded-lg p-2 text-muted-foreground hover:bg-secondary md:hidden" data-testid="button-open-menu"><Menu size={20} /></button><div className="md:hidden"><BrandMark /></div><div className="hidden items-center gap-2 text-xs text-muted-foreground md:flex"><span className="mono text-[10px] uppercase tracking-[.18em]">Workspace</span><span>/</span><span className="font-semibold text-foreground">{navGroups.flatMap(g => g.items).find(i => i.href === location)?.label || (location === '/settings' ? 'Settings' : 'Overview')}</span></div></div><div className="flex items-center gap-3"><button onClick={() => setLocation('/assistant')} className="focus-ring hidden items-center gap-2 rounded-lg border border-border bg-card px-3 py-2 text-xs font-semibold text-foreground shadow-sm hover:border-primary/40 sm:flex" data-testid="button-quick-assistant"><Sparkles size={14} className="text-primary" /> Ask your co-pilot <span className="mono ml-1 text-[9px] text-muted-foreground">⌘K</span></button><div className="hidden h-7 w-px bg-border sm:block" /><button onClick={() => setLocation('/settings')} className="focus-ring grid h-9 w-9 place-items-center rounded-full bg-secondary text-xs font-bold text-secondary-foreground" data-testid="button-profile">{initials('Maya Chen')}</button><button onClick={signOut} className="hidden text-xs font-semibold text-muted-foreground hover:text-foreground sm:block" data-testid="button-sign-out">Sign out</button></div></div><main className="mx-auto max-w-[1480px] px-5 py-7 md:px-8 md:py-9">{children}</main></div>{sidebar}{mobileOpen && <button className="fixed inset-0 z-30 bg-sidebar/40 md:hidden" onClick={() => setMobileOpen(false)} aria-label="Close navigation" data-testid="button-dismiss-menu" />}</div>;
}
